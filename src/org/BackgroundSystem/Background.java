...
public class Background extends VisibleObject {
	...
	public void addToGround(BufferedImage b, int total) {
		...
		for (int i = 0; i < total; i++) {
			g.drawImage(b, (int) round(random() * (2 * b.getWidth() + image.getWidth()) - b.getWidth()), image.getHeight() - b.getHeight(),
					null);
		}
		g.dispose();
	}
	...
	public void addToMiddle(BufferedImage b, int total) {
		Graphics g = image.createGraphics();
		for (int i = 0; i < total; i++) {
			g.drawImage(b, (int) round(random() * (2 * b.getWidth() + image.getWidth()) - b.getWidth()), (int) round(random()
					* (2 * b.getHeight() + image.getHeight()) - b.getHeight()), null);
		}
		g.dispose();
	}
}