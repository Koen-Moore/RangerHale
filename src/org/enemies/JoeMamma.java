...
public class JoeMamma extends Enemy implements Runnable{
	private static BufferedImage[]ani=new BufferedImage[]{
		ImagePack.getImage("joeMamma1.png"),
		ImagePack.getImage("joeMamma2.png")
	};
	public int curFrame;
		...
		if(life>0&&!dead){
			...
			if((counter%6)==0){
				...
				//(y<p.y+p.h&&y+h>p.y&&(x+w<p.x||x>p.x+p.w)){
					//Room.projectiles.add(new EnemyProjectile(x+(left?12:18),y+25,(x+w<p.x?3:-3),0));
				//}
			}
