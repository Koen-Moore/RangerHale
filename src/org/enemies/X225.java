...
public class X225 extends Enemy {
	...
	public static final BufferedImage[] ani = new BufferedImage[]{ImagePack.getImage("Robot/Evil robomoveL.png"),
			ImagePack.getImage("Robot/Evil roboLookR.png"), ImagePack.getImage("Robot/Evil roboLookL.png"),
			ImagePack.getImage("Robot/Evil robomoveR.png"), ImagePack.getImage("Robot/Evil robostanding.png"),
	...
	public void run() {
		...
		if (proDelay == 0 && counter % 125 < 80) {
			...
			switch (curPro) {
				...
				case 1 : {
					...
					Room.projectiles.add(new EnemyFireball(px, py - 2, pvx - svx / 2, pvy - svy / 2));
					break;
				}
				case 2 :
					Room.projectiles.add(new EnemyBigFireBall(px, py - 6, pvx - svx / 2, pvy - svy / 2));
					break;

				case 3 :
					Room.projectiles.add(new EnemyIceBullet(px, py - 1, pvx, pvy));
					break;
				case 4 :
					Room.projectiles.add(new EnemyWaterBubble(px, py, pvx, pvy));
					break;
				case 5 : {
					proDelay = 12;
					Room.projectiles.add(new EnemyRockProjectile(px, py - 3, pvx, pvy + .5f));
					break;
				}
				case 6 : {
					proDelay = 12;
					Room.projectiles.add(new EnemySuperRock(px, py - 8, pvx, pvy + .5f));
					break;
				}
				case 7 :
					Room.projectiles.add(new EnemyAirBall(px, py - 7, pvx - svx / 2, pvy - svy / 2));
					break;
				case 8 :
					Room.projectiles.add(new EnemyLightning(dx, dy, 4 * (pvx - svx), 4 * (pvy - svy), 5));
					break;
				case 9 : {
					proDelay = 2;
		...
		if (standDelay == 0
				&& (Collisions.collides(new Wall(x, 0, w, Room.HEIGHT), p) || Collisions.collides(new Wall(0, y, Room.WIDTH, h), p))) {
			standDelay = 60;
		}
		if (standDelay == 10) {
