...
public class FireBossEasy extends Enemy {
	int deathDelay = -1;
	int hitDelay = 0;
	boolean left;
	int proDelay = 0;
	int counter;
	public static final BufferedImage[] ani = new BufferedImage[]{ImagePack.getImage("firelevel/bobStand.png"),
			ImagePack.getImage("firelevel/bobStandR.png"), ImagePack.getImage("firelevel/bobHit.png"),
			ImagePack.getImage("firelevel/bobShootL.png"), ImagePack.getImage("firelevel/bobShootR.png"),
			ImagePack.getImage("firelevel/bobMove.png"), ImagePack.getImage("firelevel/bobMoveR.png"),
			ImagePack.getImage("firelevel/bobDeath.png"),};
	public FireBossEasy(int X, int Y) {
		x = X;
		y = Y;
		w = 40;
		h = 60;
	...
	public void run() {
		...
		if (life > 100) {
			...
			if (counter % 60 < 30) {
				...
				if (counter % 30 == 0) {
					float a = (x) + (17), b = y + 15;
					Room.projectiles.add(new EnemyLaserBullet(x + 8, y + 15, (dx < 0 ? -5 : 5), (float) ((dx < 0 ? 5 : -5)
							* (p.y + p.h / 2 - b) / (p.x + p.w / 2 - a))));
					Room.projectiles.add(new EnemyLaserBullet(x + 23, y + 15, (dx < 0 ? -5 : 5), (float) ((dx < 0 ? 5 : -5)
							* (p.y + p.h / 2 - b) / (p.x + p.w / 2 - a))));
					proDelay = 10;
				}
			}
