...
public class AudioPack implements Runnable {
	...
	public static Clip newClip(String path, double v) {
		...
		try {
			...
			/*
			 * byte[]bites=new byte[(int)inputStream.getFrameLength()]; for(int
			 * i=0;i<bites.length||true;i++){ byte b=(byte)inputStream.read();
			 * if (b==-1)break; //b=((byte)round((double)b*1.1)); bites[i]=b; }
