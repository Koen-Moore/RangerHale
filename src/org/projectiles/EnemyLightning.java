...
public class EnemyLightning extends Projectile implements Runnable{
	private double endX,endY;
	private Double a,b;
	public EnemyLightning(Double X,Double Y,double vx,double vy,int branches){
		a=X;b=Y;
		x=(float)(double)X;y=(float)(double)Y;
		...
		if(closestEnemy==null){
			endX=x+vx*5+2*random()-1;endY=y-vy*5+2*random()-1;
			//if(branches==0){
				//return;
			//}
			...
			//Room.projectiles.add(new Lightning(endX,endY,avg*cos(finangle)-(6*random()-3),(vy<0?-1:1)*avg*sin(finangle)+(6*random()-3),branches-1));
			//Room.projectiles.add(new Lightning(endX,endY,avg*cos(finangle)+(6*random()-3),(vy<0?-1:1)*avg*sin(finangle)-(6*random()-3),branches-1));
		}
		//out.println(vX+" "+vY);
		
		
		vX=(float)endX-x;vY=(float)(endY-y);
		...
		/*else if(closestEnemy!=null){
			...
			if(Collisions.collides(new Wall((float)endX,(float)endY,0,0), en)){
				Player.damage(branches/2+3);
				Player.vMultiplier=.01f;
			}
		...
		Room.projectiles.add(new EnemyLightning(endX,endY,vx+(6*random()-3),vy+(6*random()-3),branches-1));
		Room.projectiles.add(new EnemyLightning(endX,endY,vx+(6*random()-3),vy+(6*random()-3),branches-1));
