...
public class EnemyRockProjectile extends Projectile {
	public static final BufferedImage[][] ani=rockani;
	public int counter=(int)(100*random());
	public EnemyRockProjectile(float X,float Y,float vx,float vy){
	...
	public void run(){
		...
		if(life!=0){
			...
			int p=(counter/12)%4;
			image=ani[counter%4][0];
			if(life>-1)image=ani[p][1];
			if(life>2)image=ani[p][2];
			if(life>4)image=ani[p][3];
			if(life>6)image=ani[p][4];
			if(life>8)image=ani[p][5];
			...
			for (int i=0;i<Room.walls.size();i++){
    			...
    			if (Collisions.collides(this, wal)){
					collided=true;
    				if (wal.damagable){((DamageableWall)wal).life-=20;
