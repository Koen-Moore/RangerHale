...
public class EnemyBullet extends Projectile implements Runnable {
	...
	public EnemyBullet(float X, float Y, float vx, float vy) {
		...
		main : for (int i = 0; i < 2; i++) {
			VisibleObject[] it;
			it = (i == 0 ? Room.walls.toArray(new VisibleObject[0]) : new VisibleObject[]{Player.player});
		...
		if (closest == null) {
			...
			x = xhit;
			y = yhit;
			if (closest instanceof DamageableWall)
				((DamageableWall) closest).life -= 5;
	...
	public void draw(Graphics g) {
		...
		if ((x - ax) * (vX > 0 ? 1 : -1) < 0 || (y - ay) * (vY > 0 ? 1 : -1) < 0) {
			ax = round(x);
			ay = round(y);
			dead = true;
		}
		if ((x - bx) * (vX > 0 ? 1 : -1) < 0 || (y - by) * (vY > 0 ? 1 : -1) < 0) {
			bx = round(x);
			by = round(y);
		}
