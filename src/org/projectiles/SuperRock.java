...
public class SuperRock extends Projectile {
	public static final BufferedImage[] ani = bigrockani;
	public int counter = (int) (100 * random());
	public SuperRock(float X, float Y, float vx, float vy) {
		image = ani[counter % 8];
		dead = false;
		life = lifeCapacity = -1;
		h = 16;
			...
			synchronized (Room.enemies) {
				for (VisibleObject en : Room.enemies) {
					if (Collisions.collides(this, en)) {
						collided = true;
						((Enemy) en).damage(Element.EARTH, 40);
					}
				}
				if (collided) {
					double offset = Math.PI * ((80 * random()) % 8) / 4;
					Room.projectiles.add(new RockProjectile(x, y, (float) cos(offset) * 3, (float) sin(offset) * 3));
					Room.projectiles.add(new RockProjectile(x, y, (float) cos(offset + PI * 2 / 3) * 3,
							(float) sin(offset + PI * 2 / 3) * 3));
					Room.projectiles.add(new RockProjectile(x, y, (float) cos(offset + PI * 4 / 3) * 3,
							(float) sin(offset + PI * 4 / 3) * 3));
