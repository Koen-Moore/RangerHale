...
public class Icebullet extends Projectile {
	...
	public Icebullet(float X,float Y,float vx,float vy){
		image=ani[0];
		dead=false;
		life=lifeCapacity=-1;
		w=8;h=5;
		if(abs(vx)>abs(vy)){h=8;w=5;}
		color=red;
		x=X;y=Y;vX=vx*1.3f;vY=vy*1.3f;
		int counter=0;
	...
	public void run(){
		...
		if(life!=0){
			if(life>-1)image=ani[11];
			if(life>3)image=ani[10];
			if(life>5)image=ani[9];
			if(life>8)image=ani[8];
			//Clock.waitFor(frame=!frame);
			//if(Clock.dead)break;
			if (life<0){x+=vX;y-=vY;}
			life--;
			/*if (!(x>0&&x<Jump.WIDTH&&y>0&&y<Jump.HEIGHT)&&life<0){vY=vX=0; life=60; w=h=16;
				x-=round(10*random());
				y-=round(10*random());
			...
			for (int i=0;i<Room.walls.size();i++){
    			...
    			if ((x<wal.x+wal.w&&x+w>wal.x)&&(y<wal.y+wal.h&&y+h>wal.y)){
    				vY=vX=0; life=30+(int)(60*random()); w=h=16;image=ani[8];x-=round(10*random());
    				y-=round(10*random());
    				//AudioPack.playAudio("BExplosion2.wav",0.05);
					AudioPack.playClip(boom);
    		...
    		for (VisibleObject en:Room.enemies){
    			if((x<en.x+en.w&&x+w>en.x)&&(y<en.y+en.h&&y+h>en.y)){
    				if(life<0)
	    				((Enemy)en).damage(Element.WATER,1);
    				if(10*random()<=1)
    					((Enemy)en).damage(Element.WATER,1);
	    			((Enemy)en).vMultiplier=.4f;
	    			if(vX!=0||vY!=0){
	    				//x=en.x+en.w/2;
	    				image=ani[8];
	    				vY=vX=0; life=30+(int)(60*random()); w=h=16; 
	    				//y=en.y+en.h/2;
