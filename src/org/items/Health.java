...
public class Health extends LivingObject implements Runnable{
	public static final BufferedImage []ani=new BufferedImage[]{
		ImagePack.getImage("health.png"),
		ImagePack.getImage("healthup.png"),
	};
	...
	public Health(float X,float Y,float VX,float VY){
		...
		dead=false;
		x=X;y=Y;
		vx=(float)(VX+random()-.5);vy=(float)(VY+random()-.5);
		w=4;h=4;
		if(Math.random()<.03)healthup=true;
		image=ani[healthup?1:0];
	}
	public void run(){
		if(!free&&!Collisions.collides(this,Player.player))free=true;
