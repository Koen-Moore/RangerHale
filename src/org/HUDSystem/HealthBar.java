...
public class HealthBar extends VisibleObject {
	...
	public void draw(Graphics g) {
		...
		// out.println(Player.player.life+" "+
		// target.life+" "+((w-1)*(target.life/target.lifeCapacity)));
		g.fillRect(round(x), round(y) + 1, (int) ((w - 1) * ((double) target.life / target.lifeCapacity)), h - 2);
	}
	/*
	 * public void reload(){ //image=new BufferedImage(w,h,2); Graphics
	 * g=image.createGraphics(); g.setColor(color); g.drawRect(-1, 0, w, h-1);
	 * //out.println(Player.player.life+" "+
	 * target.life+" "+((w-1)*(target.life/target.lifeCapacity))); g.fillRect(0,
	 * 0, (int)((w-1)*((double)target.life/target.lifeCapacity)), h);
	 * g.dispose(); }
	 */
}
